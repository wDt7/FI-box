...
def rate_level_fig(hb_base,end_day):
    ...
    plt.style.use({'font.size' : 12}) 
    fig,ax = plt.subplots(ncols=2,nrows=9,figsize=(6*2,4*9), dpi=300)
...
def tmp():
    rates = do.get_data('rates'); rates.index = rates.date
    dur = pd.read_excel('/Users/wdt/Desktop/tpy/Modules/signals/久期相关/dur_v3/dur_v3.xlsx',
                        index_col=0)
    from signals.senti import cal_senti_indices, sentiment_factors
    ...
    from signals.senti import lqd_factors,cal_liquid_indice
    ...
    df_senti_score_5['交易动能得分']['2021':].plot.area(
        ax=ax[0,0],stacked=False,title='交易动能得分(级别：周)')
    ax[0,0].grid(linestyle = '--',linewidth =1, color= 'gray',alpha = 0.4,axis='x') 
    df_lqd_score_5['资金面得分']['2021':].plot.area(
        ax=ax[0,1],stacked=False,title='资金面得分(级别：周)')
    ax[0,1].grid(linestyle = '--',linewidth =1, color= 'gray',alpha = 0.4,axis='x') 
    
    do.LLT(df_senti['gz30'].dropna(),20).plot(ax=ax[1,0],label='gz30Y_turn')
    do.LLT(df_senti['gz30'].dropna(),20)['2021':].plot(ax=ax[1,1],label='gz30Y_turn')
    do.LLT(df_senti['gk10_top3'].dropna(),20).plot(ax=ax[2,0],label='gk10Y_turn')
    do.LLT(df_senti['gk10_top3'].dropna(),5)['2021':].plot(ax=ax[2,1],label='gk10Y_turn')
    ...
    # lev
    # TODO 月度修改
    ax[3,0].plot(df_lqd.lev['2021-12'].tolist(),label='12月')
    ax[3,0].plot(df_lqd.lev['2022-01'].tolist(),label='1月')
    ax[3,0].plot(df_lqd.lev['2022-02'].tolist(),label='2月')
    df_lqd.lev['2020':].rolling(5).mean().plot(
    ...
    do.LLT(dur[dur>0].std(axis=1)/dur[dur>0].mean(axis=1),10)['2021-01':].plot(ax=ax[4,1],\
        title='基金久期变异系数(10日平滑)')
    ...
    ((dur[dur>0] >= dur[dur>0].rolling(250).quantile(0.90)).sum(axis=1)/dur.count(axis=1))['2021':].\
    ...
    ax[1,0].set_title('30Y国债换手率(20-30Y),LLT20');ax[1,0].set_ylabel('%')
    ax[2,0].set_title('10Y国开换手率(top3活跃券),LLT20');ax[2,0].set_ylabel('%')
    ax[1,1].set_title('30Y国债换手,LLT20')
    ax[2,1].set_title('10Y国开换手,LLT5')
    ...
    ## spr corr
    ax[6,0].bar(df_senti.corr5['2021-06':].index,df_senti.corr5['2021-06':])
    ax[6,0].set_title('国开债期限偏好指数(周)',fontsize=16)
    do.set_axes_rotation(ax[6,0],30)
    ax[6,1].bar(df_senti.corr20['2021-06':].index,df_senti.corr20['2021-06':])
    ...
    ax[10,0].plot(df_cb.loc['2021':,'bmk'])
    ax[10,0].set_title('可转债市场-等权指数')
    ax[10,1].plot(df_cb.loc['2021':,'convp'])
    ax[10,1].set_title('市场百元溢价率')
    ax[11,0].plot(df_cb.loc['2021':,'amt'])
    ax[11,0].set_title('市场总成交额')
