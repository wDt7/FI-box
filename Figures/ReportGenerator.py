...
# 时间设置
#去极值
def winsorize_series(se,min = 0.05,max = 0.95): #百分位法
    se = se.sort_values()
...
class weeklyReport:
    def __init__(self, isMonth = False):
        ...
        self.title="周报"
        self.weekly_path = '/Users/wdt/Desktop/tpy/报告输出/周报/周报图片输出地址/'
        self.monthly_path = '/Users/wdt/Desktop/tpy/报告输出/周报/月报图片输出地址/'
    ...
    def print_all_jpg(self):
        if self.isMonth:
            download_path = self.monthly_path
        if not self.isMonth:
            download_path = self.weekly_path
    ...
    def monetary_policy_tools(self,base_day,end ):
        ...
        oneweek_list = [df.index[0] ]; df_weekly = pd.DataFrame([])
        for i in range(1, df.shape[0]):
            dat = df.index[i]
            dat_pre = df.index[i-1]
            if dat - dt.timedelta(days=1) == dat_pre:
                oneweek_list.append(dat)
            else:
                df_weekly.loc[oneweek_list[-1], 'MLF-逆回购-国库现金_回笼量'] = \
                    df.loc[oneweek_list, dql].sum().sum()
                df_weekly.loc[oneweek_list[-1], 'MLF-逆回购-国库现金_投放量'] = \
                    df.loc[oneweek_list, tfl].sum().sum()
                oneweek_list = [dat]
            if i == df.shape[0]-1:
                df_weekly.loc[oneweek_list[-1], 'MLF-逆回购-国库现金_回笼量'] = \
                    df.loc[oneweek_list, dql].sum().sum()
        ...
        if self.isMonth=='no':
            ...
            plt.annotate('本周' + net_investment(df_weekly['MLF-逆回购-国库现金_净投放量'][-1]),
                         ...
                         xytext=(df_weekly.index[-12],df_weekly['MLF-逆回购-国库现金_净投放量'][-1]-7000),color="k",weight="bold",alpha=0.9,arrowprops=dict(arrowstyle="-",connectionstyle="arc3",color="k",alpha=0.9),bbox={'facecolor': 'lightsteelblue', 'edgecolor':'k','alpha': 0.9,'pad':2},fontsize=7)
    ...
    def net_buy_amt(self):
        ...
        for windows in [10,20]:
            ...
            for co in co_list[:8]:
                ...
                stat_table3 = pd.DataFrame(index = df_co.date.unique())
                ...
                for name , grp in df_co.groupby(['期限']):
                    ...
                    stat_table3.loc[:, name+'信用债'] = \
                        grp.groupby(['date'])['中期票据','短期/超短期融资券', '企业债'].sum().sum(axis=1).tolist()
                ...
                # pd.Series.rolling(stat_table3['1-3年信用债'],window=windows).mean().plot(\
                #     ax = ax[1][1],label='1-3年信用债'+'(MA{})'.format(str(windows)))
                # ax[1][1].axhline(y=0, c="y", ls="--", lw=1.5)
                # ax_right = ax[1][1].twinx()
                # stat_table[['十年期国债收益率']].plot(ax = ax_right ,color = 'red')
                # ax[1][1].set_title('1-3年信用债')
                # ax[1,1].legend(ncol=1,loc=3, bbox_to_anchor=(0,-0.3),borderaxespad = 0.)
                # ax_right.legend(ncol=1,loc=3, bbox_to_anchor=(0.78,-0.3),borderaxespad = 0.)
                plt.delaxes(ax[1,1])
    ...
    def secondary_credit(self,start,end,lag=2):
        ...
        date = CreditBondTrading_stat.index.strftime('%m-%d')
        ...
        ax_.set_xticks([date[i] for i in range(0,len(date),lag)])
        do.set_axes_rotation(ax)
        ax_.legend(ncol=3,loc=3, bbox_to_anchor=(0.6,-0.65),borderaxespad = 0.,fontsize=10,frameon=False)
        ax.set_title('信用债 均价&笔数',fontsize=12)
        self.pic_list.append(fig1)
        ...
        do.set_axes_rotation(ax)
        ax.set_xticks([date[i] for i in range(0,len(date),lag)])
        ax_.legend(ncol=3,loc=3, bbox_to_anchor=(0.5,-0.65),borderaxespad = 0.,fontsize=10,frameon=False)
        ax.set_title('信用债 风险偏好&情绪指数',fontsize=12)
        self.pic_list.append(fig2)
        ...
        do.set_axes_rotation(ax)
        ax_.legend(ncol=3,loc=3, bbox_to_anchor=(0.6,-0.65),borderaxespad = 0.,fontsize=10,frameon=False)
        ax.set_title('信用债 平均期限&分歧指数',fontsize=12)
        self.pic_list.append(fig3)
        self.title_list.append('信用债 平均期限&分歧指数')
    ...
    def secondary_rate(self,start,end):
        ...
        plt.twinx()
        plt.plot(date,df_200215['mean'],'#3778bf',label=gk1+"均价")
        plt.plot(date,df_210205['mean'],'#f0833a',label=gk2+"均价")
        plt.legend(ncol=3,loc=3, bbox_to_anchor=(0.5,-0.7),borderaxespad = 0.,fontsize=10,frameon=False)
        plt.title(gk1+'&'+gk2,fontsize=12)
        self.pic_list.append(fig1)
        self.title_list.append(gk1+'&'+gk2)

        
        #筛选200016
        df_200016 = df.loc[df['代码'] == gz]
        ...
        plt.title(gz,fontsize=12)
        self.pic_list.append(fig2)
        self.title_list.append(gz)
    ...
    def r_dr(self,start = '2019-07-01'):
        ...
        ax.fill_between(df.date, 0, df['R007-DR007'].rolling(1 ).mean()*100, \
        ...
        # fig.savefig('r_dr.jpg',dpi=300,bbox_inches='tight')
    ...
    def fig_net_data_2(self,start,end):
        ...
        # ax.set_xticklabels(['农村金融机构', '证券公司','保险公司', '基金公司','外资银行'])
        if self.isMonth:
            t = '分机构7-10年政金债、国债月度净买入情况'
        else:
            t = '分机构7-10年政金债、国债周度净买入情况'
        ax.set_title(t, fontsize=12)
        self.pic_list.append(fig2)
...
class MacroReport:
    ...
    def fig_industrial_production(self):
        ...
        fig, ax = plt.subplots(nrows=3,ncols=2,figsize = (12,4*3), dpi=100)
        ## p1 PTA产业负荷率
        ...
        fig.delaxes(ax[1,1])
        ## p44
        data[['开工率:汽车轮胎:半钢胎']].dropna(axis = 0).plot(ax = ax[2,0])
        data[['开工率:汽车轮胎:全钢胎']].dropna(axis = 0).plot(ax = ax[2,1])
        ax[2,0].set_title('开工率:汽车轮胎:半钢胎')
        ax[2,1].set_title('开工率:汽车轮胎:全钢胎')
        ...
        for i in range(ax.shape[0]):
            for j in range(ax.shape[1]):
                ax[i,j].set_xlabel('')
        plt.suptitle("工业生产",fontsize=30,y=1.01)
        plt.tight_layout()
        self.pic_list.append(fig)
        return data
    ...
    def fig_cpi_ppi_related(self):
        ...
        for i in range(ax.shape[0]):
            for j in range(ax.shape[1]):
                ax[i,j].set_xlabel('')
        plt.suptitle('物价（CPI/PPI相关）',fontsize=30,y=1.01)
        plt.tight_layout()
        """
        # 画图 -v1
        ## fig1 
    ...
    def fig_upstream(self):
        ...
        for i in range(ax.shape[0]):
            for j in range(ax.shape[1]):
                ax[i,j].set_xlabel('')
        plt.suptitle('上游',fontsize=30,y=1.01)
        plt.tight_layout()
        self.pic_list.append(fig)
        return data
    ...
    def fig_midstream(self):
        ...
        for i in range(ax.shape[0]):
            for j in range(ax.shape[1]):
                ax[i,j].set_xlabel('')
        plt.suptitle('中游',fontsize=30,y=1.01)
        plt.tight_layout()
        self.pic_list.append(fig)
        return data
    ...
    def fig_downstream(self):
        ...
        # * 处理成月度值 用于计算同比
        data_cumtb = do.get_data('fig_downstream', '2001-01-01')
        data_cumtb.index = data_cumtb.date
        # [['30大中城市:商品房成交面积','当周日均销量:乘用车:厂家零售','100大中城市:成交土地面积']]
        data_cumtb['month_idx'] = data_cumtb.index.strftime('%Y-%m')
        data_monthly = pd.DataFrame(index=data_cumtb.month_idx.unique())
        for idx in data_monthly.index:
            # 当月均值
            data_monthly.loc[idx,'商品房'] = data_cumtb['30大中城市:商品房成交面积'][idx].dropna().mean()
            data_monthly.loc[idx,'汽车'] = data_cumtb['当周日均销量:乘用车:厂家零售'][idx].dropna().mean()
            data_monthly.loc[idx,'土地'] = data_cumtb['100大中城市:成交土地面积'][idx].dropna().mean()
            
            # 当年月度累计
            data_monthly.loc[idx, '商品房_cum'] = \
                data_cumtb['30大中城市:商品房成交面积'][idx[:4] : idx].sum()
            data_monthly.loc[idx, '汽车_cum'] = \
                data_cumtb['当周日均销量:乘用车:厂家零售'][idx[:4] : idx].sum()
        
        # fig, ax = plt.subplots(nrows=1,ncols=2,figsize = (14,4), dpi=100)
        # data_monthly['商品房_cum'].pct_change(periods=12)['2019':].plot(ax=ax[0],\
        ...
        # 画图
        fig, ax = plt.subplots(nrows=9,ncols=2,figsize = (12,4*9), dpi=100)
        
        data_monthly['商品房'].pct_change(12)['2009':].plot(
            ax=ax[0,0],label='商品房成交面积：同比(月)',
            title='商品房成交面积：同比(月)')
        ax[0,0].set_ylabel('%')
        data_monthly['土地'].pct_change(12)['2009':].plot(
            ax=ax[0,1],label='土地成交面积：同比(月)',
            title = '土地成交面积：同比(月)')
        ax[0,1].set_ylabel('%')
        ...
        ## P5
        data[['柯桥纺织:价格指数:总类']].dropna(axis=0).plot(ax=ax[4,0])
        ax[4,0].set_title('纺织服装价格')
        ## P6
        data[['义乌中国小商品指数:总价格指数']].dropna(axis=0).plot(ax=ax[4,1])
        ax[4,1].set_title('商贸批发零售')
        ## P7
        data[['电影票房收入']].dropna(axis=0).plot(ax=ax[5,0])
        ax[5,0].set_title('电影票房收入和观影人次')
        ## P8
        fig.delaxes(ax[5,1])
        ## P9
        data[['中国公路物流运价指数']].dropna(axis=0).plot(ax=ax[6,0])
        ax[6,0].set_title('中国公路物流运价指数')
        ## P10
        data[['波罗的海干散货指数(BDI)']].dropna(axis=0).plot(ax=ax[6,1])
        ax[6,1].set_title('波罗的海干散货指数')
        ## P10
        data[['CCFI:综合指数']].dropna(axis=0).plot(ax=ax[7,0])
        ax[7,0].set_title('中国出口集装箱运价指数')
        ## P10
        data[['CICFI:综合指数']].dropna(axis=0).plot(ax=ax[7,1])
        ax[7,1].set_title('中国进口集装箱运价指数')
        # P6,0 
        data = do.get_data('fig_downstream', '2020-01-01' )
        data.index = data.date
        data[['韩国:出口总额:同比']].dropna(axis=0).plot(ax=ax[8,0])
        ax[8,0].set_title('韩国出口总额：同比')
        data[['沿海主要港口货物吞吐量:当月值']].dropna(axis=0).plot(ax=ax[8,1])
        ax[8,1].set_title('港口吞吐量')

        for i in range(ax.shape[0]):
            for j in range(ax.shape[1]):
                ax[i,j].set_xlabel('')
        plt.suptitle('下游',fontsize=30,y=1.01)
        plt.tight_layout()
        self.pic_list.append(fig)
