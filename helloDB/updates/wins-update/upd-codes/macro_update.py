...
def macro_month_1():
    name = 'macro_month_1'
    err, df = w.edb("M0001428", 
        "2009-01-01", "2021-11-29",usedf=True)
    df.columns = ['社零总额:同比']
    df['date'] = df.index
    columns_type = [Float(),DateTime()]
    dtypelist = dict(zip(df.columns,columns_type))
    do.upload_data(df, name , dtypelist,'replace')
